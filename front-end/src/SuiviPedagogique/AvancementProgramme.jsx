...
const AvancementProgramme = () => {
    const { affectations, seances, semaineNumber, groupes } = useContext(contextProvider);
    ...
    const calculeMHHebdoGroupe = (groupe ,semaine) => {
        let count = 0
        seances.filter(seance => seance.Code_Groupe === groupe && seance.No_Semaine_Calendrier === semaine)
            .map(e => {
                count += e.MH
            })
        return count
        ...
        <Navbar/>
                    ...
                    <div className="choisir_input_box3 select">
                        ...
                        <div onClick={() => setAfficherSelect(!afficherSelect)} className="select-btn1">
                            <span>{groupe ? groupe : "Selectioner un Groupe"}</span>
                        ...
                        </div>
                        {
                            ...
                            <div className="content">
                                ...
                                <ul className="options1">
                                    {
                                        groupes.map((s,index) => (
                                            <li onClick={() => handleSelected(s.Code_Groupe)} key={index}>
                                                {s.Code_Groupe} 
                                            </li>
                                        ))
                                    }
                    ...
                    <table>
                        ...
                        <tbody>
                            { groupe? 
                                ...
                                {
                                semaineNumber.map((semaine) => (
                                        <td>{calculeMHHebdoGroupe(groupe.Code_Groupe,semaine.number)}</td>
